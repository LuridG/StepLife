...
class BasketStats {
  /// 按时间升序的单价序列（过滤无效价格）
  static List<double> priceSeries(List<StoreLog> logs) {
  ...
  static List<({StoreLog log, double price})> pricePoints(List<StoreLog> logs) {
    final sorted = [...logs]..sort((a, b) => a.timestamp.compareTo(b.timestamp));
  ...
  /// 最近单价（无记录返回 null）
  static double? latestPrice(List<StoreLog> logs) {
    final s = priceSeries(logs);
    return s.isEmpty ? null : s.last;
  }

  /// 最近一次价格记录时间
  static DateTime? latestTime(List<StoreLog> logs) {
    final pts = pricePoints(logs);
    return pts.isEmpty ? null : pts.last.log.timestamp;
  }

  /// 较上次涨跌幅（比例，如 0.08 = 涨 8%）；无法计算返回 null
  static double? changeVsPrevious(List<StoreLog> logs) {
    final s = priceSeries(logs);
  ...
  static double? changeVsLastMonth(List<StoreLog> logs) {
    final pts = pricePoints(logs);
  ...
  static List<double> movingAverage(List<StoreLog> logs, {int windowDays = 7}) {
    final pts = pricePoints(logs);
  ...
  static String? trendDirection(List<StoreLog> logs, {double threshold = 0.005}) {
    final c = changeVsPrevious(logs);
