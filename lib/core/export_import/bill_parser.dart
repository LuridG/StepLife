...
只返回 JSON，不输出任何解释。
字段规则：
- records: 数组。每项含 amount(数字，单位元，去掉 ¥ ￥ 等货币符号；金额可能带负号如 -8.00，表示扣款/支出，一律取绝对值)、time(格式 yyyy-MM-dd HH:mm:ss，必须精确到秒；中文日期如「2026年8月3日 16:16:12」需转换为「2026-08-03 16:16:12」，秒必须原样保留；账单未写日期按今天 $today 推断，未写具体时间默认 12:00:00)、memo(该笔消费内容/菜名，如「商品」标签后的名称，没有则省略该字段)。
- storeGuess: 仅当账单中出现与当前店铺「$storeName」不同的店铺名时给出 {"name":"店铺名","category":"建议分类"}，否则省略。
输出格式：{"records":[{"amount":12.5,"time":"$today 12:30:00","memo":"微信支付-餐饮"},{"amount":8.0,"time":"$today 12:35:00","memo":"奶茶"}],"storeGuess":{...}}''';
